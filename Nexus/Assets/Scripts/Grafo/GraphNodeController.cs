...
public class GraphNodeController : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerDownHandler
{
    [Header("Arrastre")]
    ...
    private void InstallInHierarchy(Transform root)
    {
        for (int i = 0; i < root.childCount; i++)
        {
            ...
            if (childImage != null)
            {
                ...
                if (child.GetComponent<GraphNodeController>() == null)
                    child.gameObject.AddComponent<GraphNodeController>();
            }
    ...
    /// <summary>
    /// Identifica elementos de UI que no deben ser tocados por el sistema de grafo.
    /// </summary>
    private static bool IsUtilityObject(Transform child)
    {
        if (child == null)
            return false;

        // No interferir con el compás, el mapa ni sockets ya creados.
        if (child.name.IndexOf("Compass", StringComparison.OrdinalIgnoreCase) >= 0 ||
            child.name.StartsWith("Socket_", StringComparison.OrdinalIgnoreCase))
        {
            return true;
        }

        return child.GetComponent<Compass>() != null ||
               child.GetComponent<MapMarker>() != null ||
               child.GetComponent<RawImage>() != null ||
               child.GetComponent<Mask>() != null;
    }
    ...
    public void OnDrag(PointerEventData eventData)
    {
        ...
        if (GraphUiUtility.TryGetPointerWorldPoint(_rectTransform.parent as RectTransform, _canvas, eventData, out Vector3 worldPoint))
